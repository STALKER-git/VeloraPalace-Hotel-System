...
export default function EditRoomModal({ room, onClose, onSuccess }: EditRoomModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    type_chambre: room.type_chambre || 'standard',
    prix_base_nuit: room.prix_base_nuit?.toString() || '',
    capacite_adultes: room.capacite_adultes || 2,
    capacite_enfants: room.capacite_enfants || 0,
    etage: room.etage?.toString() || '',
    vue: room.vue || '',
    superficie_m2: room.superficie_m2?.toString() || '',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-2xl bg-[#0a0a0a] border border-gold/30 p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto hidden-scrollbar">
        ...
        <div className="mb-8">
          ...
          <h2 className="text-3xl font-display text-white">Edit Room</h2>
          <p className="text-text-muted text-xs mt-1">Updating: <span className="text-gold">{room.numero_chambre}</span></p>
        </div>
